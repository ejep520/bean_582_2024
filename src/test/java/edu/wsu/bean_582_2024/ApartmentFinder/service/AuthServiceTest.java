...
public class AuthServiceTest {
    ...
    public void authenticateUserNotFound() {
        ...
        Authentication authentication = new UsernamePasswordAuthenticationToken("nonexistent",
            "pass");
    ...
    public void registerNewUserSuccess() throws Exception {
        ...
        Method registerMethod = authServiceClass.getDeclaredMethod("register", String.class,
    ...
    public void authenticateExceptionThrown() {
        ...
        when(userRepository.getUserByUsername(anyString())).thenThrow(
            new RuntimeException("Database error"));

        // Create an instance of AuthService with the mock UserRepository
        AuthService authService = new AuthService(userRepository,
            null); // Pass null for AuthorityRepository
    ...
    */
    ...
    public void deleteNonExistentAuthority() {
        ...
        verify(authorityRepository).remove(nonExistentAuthority);
    }
    ...
    public void registerWithNullCredentials() {
        ...
        assertThrows(NullPointerException.class,
            () -> authService.register(null, "password", Role.USER),
            "Registration with null username should throw NullPointerException");
        assertThrows(NullPointerException.class,
            () -> authService.register("username", null, Role.USER),
    ...
    public void getUserCountReflectsActualCount() {
        ...
        assertEquals(42L, authService.getUserCount(),
            "getUserCount should return the actual number of users in the repository");
    }
    ...
    @Test
    public void userRegistrationTestFirstUserIsAdmin1() {
        ArgumentCaptor<User> argumentCaptor = ArgumentCaptor.forClass(User.class);
        User user = new User("testUser", "testPass", Role.USER);
        when(userRepository.count()).thenReturn(0L);
        authService.register(user);
        verify(userRepository).add(argumentCaptor.capture());
        assertEquals(Role.ADMIN, argumentCaptor.getValue().getRole());
    }

    @ParameterizedTest(name="Assert that authority {0} has {1} authorities")
    @MethodSource("roleAuthoritiesMatchStream")
    public void userRegistrationForRole(Role role, int authorityCount) {
        ...
        assertEquals(authorityCount, argumentCaptor.getValue().getAuthorities().size());
    }
    
    private static Stream<Arguments> roleAuthoritiesMatchStream() {
        return Stream.of(Arguments.of(Role.ADMIN, 3),
            Arguments.of(Role.OWNER, 2),
            Arguments.of(Role.USER, 1));
    }
}
